      ...
      >
        ...
        {debugOpen && (
          <div style={{ position:'fixed', top:44, right:4, zIndex:9999, background:'#111', border:'1px solid #ff0', borderRadius:6, padding:'6px 8px', display:'flex', flexDirection:'column', gap:4, minWidth:160, maxHeight:'80vh', overflowY:'auto' }}>
            ...
            <button onClick={() => setGameState(prev => ({ ...prev, huesin: (prev.huesin ?? 0) + 50 }))} style={{ fontSize:10, padding:'2px 4px', background:'#221100', color:'#ffb347', border:'1px solid #ffb347', borderRadius:3, cursor:'pointer' }}>+50 huesin</button>
            <button onClick={() => setGameState(prev => ({ ...prev, tutorial: { ...prev.tutorial, completed: true, currentStep: 3, staminaUnlocked: true, pickaxeUnlocked: true, minesUnlocked: true, goldPerSecondBought: true, pickaxeUpgradeDone: true, introDone: true, minesHinted: true, snacksHinted: true, automineHinted: true, forgeIntroDone: true, tavernIntroDone: true, cambistaIntroDone: true, dogsIntroDone: true, raidIntroDone: true, rentalTutorialStep: 3, sobreIntroDone: true, mineIntroBronzeDone: true, mineIntroIronDone: true, mineIntroDiamondDone: true } }))} style={{ fontSize:10, padding:'2px 4px', background:'#002200', color:'#69f080', border:'1px solid #69f080', borderRadius:3, cursor:'pointer' }}>skip tutorial</button>
            <span style={{ fontSize:9, color:'#ff0', fontWeight:800, letterSpacing:1 }}>DAILY LOGIN</span>
            <button onClick={() => setGameState(prev => ({ ...prev, dailyLogin: { lastClaimedDate: '2026-07-23', streak: 1 } }))} style={{ fontSize:10, padding:'2px 4px', background:'#220022', color:'#ff88ff', border:'1px solid #ff88ff', borderRadius:3, cursor:'pointer' }}>simular día 2</button>
            <button onClick={() => setGameState(prev => ({ ...prev, dailyLogin: { lastClaimedDate: null, streak: 0 } }))} style={{ fontSize:10, padding:'2px 4px', background:'#220022', color:'#ff88ff', border:'1px solid #ff88ff', borderRadius:3, cursor:'pointer' }}>reset daily</button>
            <span style={{ fontSize:9, color:'#ff0', fontWeight:800, letterSpacing:1 }}>MINEROS</span>
            <button onClick={() => setGameState(prev => { const updated = { ...prev.dogs }; MINER_DOGS.forEach(id => { if (updated[id]) updated[id] = { ...updated[id], hired: true, fragments: 999, stars: 0 }; }); return { ...prev, dogs: updated }; })} style={{ fontSize:10, padding:'2px 4px', background:'#331100', color:'#ff9944', border:'1px solid #ff9944', borderRadius:3, cursor:'pointer' }}>desbloquear + 0★</button>
            {MINER_DOGS.map(id => (
            ...
            <span style={{ fontSize:9, color:'#ff0', fontWeight:800, letterSpacing:1, marginTop:4 }}>TABLÓN</span>
            <button onClick={() => setGameState(prev => ({ ...prev, debugDayOffset: (prev.debugDayOffset ?? 0) + 1 }))} style={{ fontSize:10, padding:'2px 4px', background:'#220033', color:'#cc88ff', border:'1px solid #cc88ff', borderRadius:3, cursor:'pointer' }}>+1 día (rotación)</button>
            <button onClick={() => setGameState(prev => ({ ...prev, debugDayOffset: 0 }))} style={{ fontSize:10, padding:'2px 4px', background:'#220033', color:'#cc88ff', border:'1px solid #cc88ff', borderRadius:3, cursor:'pointer' }}>reset día</button>
            <span style={{ fontSize:9, color:'#ff0', fontWeight:800, letterSpacing:1, marginTop:4 }}>FORJA</span>
            <button onClick={() => setGameState(prev => { const updated = { ...prev.forgeDogs }; FORGE_DOGS.forEach(id => { if (updated[id]) updated[id] = { ...updated[id], hired: true, fragments: 999, stars: 0 }; }); return { ...prev, forgeDogs: updated }; })} style={{ fontSize:10, padding:'2px 4px', background:'#331100', color:'#ff9944', border:'1px solid #ff9944', borderRadius:3, cursor:'pointer' }}>desbloquear + 0★</button>
            <button onClick={() => setGameState(prev => { const updated = {}; Object.keys(prev.furnaces).forEach(mat => { updated[mat] = { ...prev.furnaces[mat], unlocked: false, isActive: false, progress: 0, startTime: null, restarting: false }; }); return { ...prev, furnaces: updated }; })} style={{ fontSize:10, padding:'2px 4px', background:'#331100', color:'#ff9944', border:'1px solid #ff9944', borderRadius:3, cursor:'pointer' }}>bloquear todos los hornos</button>
            <button onClick={() => setGameState(prev => { const updated = {}; Object.keys(prev.furnaces).forEach(mat => { updated[mat] = { ...prev.furnaces[mat], level: 1 }; }); return { ...prev, furnaces: updated }; })} style={{ fontSize:10, padding:'2px 4px', background:'#331100', color:'#ff9944', border:'1px solid #ff9944', borderRadius:3, cursor:'pointer' }}>quitar mejoras hornos</button>
            {FORGE_DOGS.map(id => (
              <div key={id} style={{ display:'flex', alignItems:'center', gap:4, fontSize:11, color:'#fff' }}>
                <span style={{ width:70 }}>{id}</span>
